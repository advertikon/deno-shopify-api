...
export class ShopifyApi extends EventEmitter {
  ...
  static oauthPage(
    shop: string,
    { redirectUrl, scopes, clientId, nonce }: OauthContext
  ) {
    nonce = nonce ?? ulid();
  ...
  on(
    eventName: "error",
    listener: (data: { status: number; url: string; body: any }) => void
  ): this;
  on(
  ...
  async getProducts(
    ...
    return this.send<{ products: ShopifyProduct[] }>(url).then(
  ...
  async getWebhooks(options: GetWebhookOptions = {}) {
    ...
    return this.send<{ webhooks: ShopifyWebhook[] }>(url).then(
      (resp) => resp.webhooks
    );
  }
  ...
  async getRecurringCharges(): Promise<ShopifyRecurringCharge[]> {
    ...
    return this.send<{
      recurring_application_charges: ShopifyRecurringCharge[];
    }>(url).then((resp) =>
      resp.recurring_application_charges.filter(
  ...
  async createRecurringCharge(
    options: CrateRecurringChargeOptions
    ...
    return this.send<{ recurring_application_charge: ShopifyRecurringCharge }>(
      url,
      {
        method: "POST",
        body,
  ...
  async getResourceMetafields(
    ...
    const url = `/${resourceType}/${resourceId}/metafields.json?${MakeQueryString(
      options
    )}`;
    return this.send<{ metafields: ShopifyMetafield[] }>(url).then(
      (resp) => resp.metafields
    );
  }
  ...
  async getResourceMetafield(
    ...
    const url = `/${resourceType}/${resourceId}/metafields/${metafieldId}.json?${MakeQueryString(
      options
    )}`;
    return this.send<{ metafield: ShopifyMetafield }>(url).then(
      (resp) => resp.metafield
    );
  }
  ...
  private async paginate<T extends PaginateAbleEntity>(
    ...
    while (count++ < 100) {
      const limit = totalCount
        ? Math.max(1, Math.min(batchSize, totalCount - resultsCount))
        : batchSize;
      let url =
        targetUrl.indexOf("?") > 0
      ...
      const response = await retry(
        () => fetch(apiUrl, options).then(this.checkForRetry),
        {
          maxAttempts: 3,
        }
      );

      const res = await this.processResponse<Record<string, T[]>>(
        response,
        apiUrl
      );
      ...
      const hasMore =
        Number(response.headers.get("link")?.indexOf('rel="next"')) > 0;
